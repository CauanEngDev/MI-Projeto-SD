...
module motor_background #(
	...
	// Novo: permite iniciar automaticamente um novo frame
	// quando o frame anterior terminou.
	wire continuous_start = scroll_auto_en && (state == S_DONE);

	// Um novo frame pode ser iniciado externamente ou automaticamente.
	wire render_start = accept_start || continuous_start;
        ...
        if (reset) begin
            ...
            case (state)
                ...
                S_DONE: begin
						 ...
						 if (scroll_auto_en) begin
							  // Continua renderizando automaticamente.
							  screen_x <= 9'd0;
							  screen_y <= 8'd0;

							  // A nova posição do scroll já foi calculada
							  // através de render_start.
							  cache_valid <= 1'b0;
