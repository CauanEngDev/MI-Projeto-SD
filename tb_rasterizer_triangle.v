...
module tb_rasterizer_triangle;
    ...
    integer   color_errors;
    integer   i;

    // Cor esperada para o desenho em andamento, resolvida diretamente pelo
    // testbench a partir de palette_mem -- usada para conferir, a cada
    // escrita no framebuffer, que fb_wr_data bate com a cor da paleta.
    reg [8:0] expected_color;

    // Modelo da palette_ram externa: RAM sincrona com saida registrada,
    // 1 ciclo de latencia entre palette_rd_addr valido e palette_rd_data
    // valido -- mesmo timing que o modulo real espera (endereco setado em
    // S_IDLE, aguardado em S_PALETTE_WAIT, lido em S_SETUP).
    ...
    always @(posedge clk) begin
        if (fb_we) begin
            ...
            if (fb_wr_data !== expected_color) begin
                $display("ERRO: cor errada em (%0d,%0d): esperado=%h obtido=%h", fb_wr_x, fb_wr_y, expected_color, fb_wr_data);
    ...
    // Recebe color_index (0-255) + palette_sel em vez do RGB direto;
    // a cor final escrita no framebuffer deve bater com palette_mem[{sel,idx}].
    task draw_triangle(
        input [8:0] ax, input [7:0] ay,
        input [8:0] bx, input [7:0] by,
        input [8:0] cx, input [7:0] cy,
        input [7:0] c_idx, input c_sel
    );
        begin
            clear_rows;
            @(posedge clk);
            ...
            color_index <= c_idx;
            palette_sel <= c_sel;
            expected_color <= palette_mem[{c_sel, c_idx}];
            start <= 1'b1;
            @(posedge clk);
            start <= 1'b0;
            wait (done == 1'b1);
    ...
    initial begin
        ...
        color_index = 0; palette_sel = 0;
        clear_rows;

        // Popula a paleta simulada com as mesmas cores puras usadas
        // nos testes originais, agora endereçadas por índice.
        palette_mem[{1'b0, 8'd0}] = 9'b111000000; // vermelho
        palette_mem[{1'b0, 8'd1}] = 9'b000111000; // verde
        palette_mem[{1'b0, 8'd2}] = 9'b000000111; // azul
        palette_mem[{1'b0, 8'd3}] = 9'b101010101;
        palette_mem[{1'b0, 8'd4}] = 9'b110011001;
        palette_mem[{1'b0, 8'd5}] = 9'b011011011;
